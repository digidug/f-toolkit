...
class thumbalizrRequest {
	...
	public function __construct($config) {
		...
		$this->config = array(
			...
			'local_cache_dir' => (!isset($config['local_cache_dir'])) ? '/Applications/XAMPP/xamppfiles/htdocs/laravel/public/thumbnails' : $config['local_cache_dir'],
	...
	private function build_request($url) {
		...
		$this->filename=md5($url) .'_'. $this->config['bwidth'] .'_'. $this->config['bheight'] .'_'. $this->config['delay'] .'_'. $this->config['quality'] .'_'. $this->config['width'] .'.'. $this->config['encoding'];
		$this->local_cache_file = $this->local_cache_subdir .'/'. $this->filename;
	...
	private function save() {
		if($this->img && $this->config['use_local_cache'] === TRUE) {
